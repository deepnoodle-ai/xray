...
export function queryDom(
  ...
  if (typeof document === "undefined") {
    return { found: false, count: 0, html: null, text: null, attributes: null, boundingRect: null, visible: false };
  }
...
export function getPerformanceMetrics(): PerformanceMetrics {
  ...
  metrics.renderCount = ((window as unknown as Record<string, unknown>).__XRAY_RENDER_COUNT__ as number) || 0;
...
export function getFocusInfo(): FocusInfo {
  if (typeof document === "undefined") {
    return { activeElement: null, activeElementId: null, activeElementClasses: [] };
  }
 ...
 */
export function getAccessibilityInfo(selector?: string): Record<string, unknown> {
  ...
  const items = Array.from(interactiveElements).slice(0, 50).map((item) => {
